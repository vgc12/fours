...
namespace Board
{
    public sealed class GridData
    {
        ...
        public int ColumnsPerRow { get; private set; }

        public void Initialize(List<Square> squares, int columnsPerRow)
        {
            ColumnsPerRow = columnsPerRow;
            TotalRows = Mathf.CeilToInt((float)squares.Count / columnsPerRow);
            _grid = new Square[TotalRows, columnsPerRow];
        ...
        {
            if (_grid == null || row < 0 || row >= TotalRows || column < 0 || column >= ColumnsPerRow)
                return null;
